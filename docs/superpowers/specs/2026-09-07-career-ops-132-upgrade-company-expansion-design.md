...
The upgrade uses the repository's system updater in preserve mode. Before applying it, custom untracked code and mutable Sunny data are copied into separate dated archives outside the repository, both archives receive SHA-256 checksums and listing verification, immutable user files receive a SHA-256 manifest in `data/cache/upgrade/`, and fork-only user directories are declared in `config/local-paths.txt`. The private data archive includes `portals.yml`, the profile/config rules, `profiles/`, and all relevant DOL resolution/checkpoint state so the expansion can be reversed independently of Git. Clone-only privacy ignores go in `.git/info/exclude` so the pre-upgrade step does not create another modified system file. Private user-layer files are never committed.
...
Every input identity receives two durable v2 axes. `identity_status` is one of `already_covered`, `owner_verified`, `reviewed_official_link`, `review_required`, `owner_mismatch`, `owner_unreachable`, `unsupported_official`, `ambiguous`, `excluded`, `invalid_identity`, or `unresolved`. `health_status` is one of `live`, `live_empty`, `dead`, `partial`, `transient_error`, or `not_checked`. A board may enter `portals.yml` only when identity is `owner_verified` or `reviewed_official_link` and health is `live` or `partial`; partial means useful jobs were recovered but coverage is incomplete, not that ownership is unsafe. Repeating the same pass must add zero duplicates. A versioned checkpoint replays all old unresolved/error records once after the upgrade and supports explicit retry axes thereafter.
