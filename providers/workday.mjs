...
function makeEndpoint(origin, tenant, site, jobBase = `${origin}/${site}`) {
  return {
...
function resolveEndpoint(entry) {
  ...
  for (const url of [entry.api, entry.careers_url]) {
    ...
    const modernCxs = url.match(MODERN_CXS_RE);
    if (modernCxs) {
      const [, instance, tenant, site] = modernCxs;
      const origin = `https://${instance}.myworkdaysite.com`;
      return makeEndpoint(origin, tenant, site, `${origin}/recruiting/${tenant}/${site}`);
    }
    ...
    const modern = url.match(MODERN_CAREERS_RE);
    if (modern) {
      const [, instance, tenant, site] = modern;
      const origin = `https://${instance}.myworkdaysite.com`;
      return makeEndpoint(origin, tenant, site, `${origin}/recruiting/${tenant}/${site}`);
    }
